...
class PlantProfile(models.Model):
    ...
    mixer = models.ForeignKey(Mixer, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    def to_json(self):
        ...
        link =self.get_mixer_link(),
        if self.mixer_id:
            link=reverse('check_mixer_redirect', args=[self.pk, ])
        data = {'pk': self.pk,
    ...
    def calc_concentrates(self, pushed_element=None, val=None):
        ...
        if self.taml != 0:
            ...
            self.suma = f'Объем: {round(av,2)} мл, вес: {round(am,2)} гр, плотность: {round(ak,2)} г/мл.'
            self.lvola = f'Концентрат А ({round(ac,2)}:1). Долить воды: {round(aw,2)} мл. Разбавлять по {round(aml,2)} мл на 1л.'
        ...
        if self.tbml != 0:
            ...
            self.sumb = f'Объем: {round(bv * 10) / 10} мл, вес: {round(bm,2) } гр, плотность: {round(bk,2)} г/мл'
            self.lvolb = f'Концентрат Б ({round(bc,2)}:1) . долить воды: {round(bw,2)} мл. Разбавлять по {round(bml,2)} мл на 1л.'
        self.conc_errors()
