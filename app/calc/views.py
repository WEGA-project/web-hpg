...
def check_mixer_redirect(request, pk):
    self = PlantProfile.objects.get(pk=pk)
    current_v_volumes =self.mixer.get_current_v_values()
    print('current_v_volumes', current_v_volumes)
    params = {}
    
    if self.mixer:
        params['mixer_id'] = self.mixer.pk

        if self.m_cano3:
            if float(current_v_volumes.get(self.m_cano3)) + round(self.gg_cano3, 2) < 1000:
                params[self.m_cano3] = round(self.gg_cano3, 2)
            else:
                return HttpResponse(f"cano3 {round(self.gg_cano3, 2)} не хватает {1000-(float(current_v_volumes.get(self.m_cano3)) + round(self.gg_cano3, 2))}")
            
        if self.m_kno3:
            if current_v_volumes.get(self.m_kno3)+round(self.gg_kno3, 2) < 1000:
                params[self.m_kno3] = round(self.gg_kno3, 2)
            else:
                return HttpResponse(f"kno3 {round(self.gg_kno3, 2)} не хватает {1000-(current_v_volumes.get(self.m_kno3)+round(self.gg_kno3, 2) )}")
            
        if self.m_nh4no3:
            if current_v_volumes.get(self.m_nh4no3)+round(self.gg_nh4no3, 2) < 1000:
                params[self.m_nh4no3] = round(self.gg_nh4no3, 2)
            else:
                return HttpResponse(f" nh4no3 { round(self.gg_nh4no3, 2)} не хватает  {1000-(current_v_volumes.get(self.m_nh4no3)+round(self.gg_nh4no3, 2) )}")
            
        if self.m_mgno3:
            if current_v_volumes.get(self.m_mgno3)+round(self.gg_mgno3, 2) < 1000:
                params[self.m_mgno3] = round(self.gg_mgno3, 2)
            else:
                return HttpResponse(f"mgno3 {round(self.gg_mgno3, 2)} не хватает  {1000-(current_v_volumes.get(self.m_mgno3)+round(self.gg_mgno3, 2) )}")
            
        if self.m_cacl2:
            if current_v_volumes.get(self.m_cacl2)+round(self.gg_cacl2, 2) < 1000:
                params[self.m_cacl2] = round(self.gg_cacl2, 2)
            else:
                return HttpResponse(f"cacl2 {round(self.gg_cacl2, 2)} не хватает  {1000-(current_v_volumes.get(self.m_cacl2)+round(self.gg_cacl2, 2) )}")
            
        if self.m_mgso4:
            if current_v_volumes.get(self.m_mgso4)+round(self.gg_mgso4, 2) < 1000:
                params[self.m_mgso4] = round(self.gg_mgso4, 2)
            else:
                return HttpResponse(f"mgso4 {round(self.gg_mgso4, 2)} не хватает {1000-(current_v_volumes.get(self.m_mgso4)+round(self.gg_mgso4, 2) )}")
            
        if self.m_kh2po4:
            if current_v_volumes.get(self.m_kh2po4)+round(self.gg_kh2po4, 2) < 1000:
                params[self.m_kh2po4] = round(self.gg_kh2po4, 2)
            else:
                return HttpResponse(f" kh2po4 { round(self.gg_kh2po4, 2)} не хватает {1000-(current_v_volumes.get(self.m_kh2po4)+round(self.gg_kh2po4, 2) )}")
            
        if self.m_k2so4:
            if current_v_volumes.get(self.m_k2so4)+round(self.gg_k2so4, 2) < 1000:
                params[self.m_k2so4] = round(self.gg_k2so4, 2)
            else:
                return HttpResponse(f"k2so4 {round(self.gg_k2so4, 2)} не хватает {1000-(current_v_volumes.get(self.m_k2so4)+round(self.gg_k2so4, 2) )}")
    
        if self.micro_calc_mode == self.CalcMicroMode.B:
            if current_v_volumes.get(self.m_cmplx) + round(self.gg_cmplx, 2) < 1000:
                params[self.m_cmplx] = round(self.gg_cmplx, 2)
            else:
                return HttpResponse(f"микро {round(self.gg_cmplx, 2)} не хватает {1000-(current_v_volumes.get(self.m_cmplx)+round(self.gg_cmplx, 2) )}")
    else:
        params[self.m_cano3] = round(self.gg_cano3, 2)
        params[self.m_kno3] = round(self.gg_kno3, 2)
        params[self.m_nh4no3] = round(self.gg_nh4no3, 2)
...
def edit_plant_profile(request, pk, micro=False):
    ...
    form = PlantProfileEditForm(instance=instance, user = request.user)
    context['form'] = form
    context['micro'] = micro
    ...
    if request.method == 'POST':
        form = PlantProfileEditForm(instance=instance, user = request.user, data=request.POST)
        if form.is_valid():
